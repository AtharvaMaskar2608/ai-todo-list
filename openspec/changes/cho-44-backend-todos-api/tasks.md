...
- [ ] 1.1 Create `backend/app/routers/__init__.py` as a package marker (empty is
...
- [ ] 2.1 Create `backend/app/routers/todos.py` with
...
- [ ] 2.2 Implement `GET /todos` → 200 `list[TodoResponse]`, delegating to
...
- [ ] 2.3 Implement `GET /todos/{id}` → 200 `TodoResponse`, calling
  ...
  miss.
- [ ] 2.4 Implement `POST /todos` with `status_code=201` → `TodoResponse`, calling
  ...
  body, 422 on invalid.
- [ ] 2.5 Implement `PUT /todos/{id}` → 200 `TodoResponse`, calling
  ...
  invalid.
- [ ] 2.6 Implement `DELETE /todos/{id}` with `status_code=204` and no
...
- [ ] 2.7 Ensure every route uses `db: Session = Depends(get_db)` and contains no
...
- [ ] 3.1 Create `backend/tests/test_todos_api.py` using FastAPI `TestClient`
...
- [ ] 3.2 Test all five endpoints and status codes: GET list (200, newest-first),
...
- [ ] 3.3 Overall done condition: all 5 endpoints pass their TestClient tests.
